...
//    System.out.println(count);
    ...
    int ss = findLastP("zzzzz");
    System.out.println(ss);

      int s = findFirstP("stop");
      System.out.println(s);

    int x = findFirstP("xxxyyyzzz");
   ...
   *   number of P's in s
   */
  private static int countP(String s) { // correct
    int count = 0;
    int i = 0;
    while (i < s.length()) {
   ...
   */
  private static int findFirstP(String s) { // "stop"
    int i = 0;
    while (i < s.length()) {
      int index = i;
      if (isLetterP(s.charAt(i))) {
